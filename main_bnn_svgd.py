...
parser.add_argument('--sampler', type=str, default='o_svgd')    
...
for _ in range(args.num_particles):
    particles.append(torch.cat([p.detach().flatten() for p in ResNet18(num_classes=num_classes_dict[args.dataset]).parameters()]).detach())
model = ResNet18(num_classes=num_classes_dict[args.dataset])
initial_particles = torch.stack(particles).cuda()
...
# Sampler 
if args.sampler == 'o_svgd':
    sampler = samplers_bnn.O_SVGD(model,initial_particles, alpha = args.alpha, beta=args.beta, wd=args.wd, 
                                        datasize=datasize,num_particles=args.num_particles,criterion=criterion)   
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        floss,gloss,outputs = sampler.step(inputs,targets,lr,epoch)
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            if use_cuda:
                inputs, targets = inputs.cuda(), targets.cuda()
            outputs = sampler.eval(inputs,targets)
